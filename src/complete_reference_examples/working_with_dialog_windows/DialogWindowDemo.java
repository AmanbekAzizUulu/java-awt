package complete_reference_examples.working_with_dialog_windows;
...
public class DialogWindowDemo extends Frame {
	...
	private class MenuHandler implements ActionListener, ItemListener { 							// Определение внутреннего класса для обработки действи в меню
		...
		public void actionPerformed(ActionEvent e) { 												// Обработка событий действия
			...
			if (action_command.equals("New")) {
				message += "New";
				dialog_window_new.setVisible(true);
			}
	...
	private class SampleDialog extends Dialog{
		private final Label label_press_to_close;
		private final Button button_press_to_close;

		public SampleDialog(Frame parentFrame, String dialogTitle) {
			super(parentFrame, dialogTitle, false);

			label_press_to_close = new Label("Press to close the dialog window");
			button_press_to_close = new Button("Close");

			button_press_to_close.addActionListener(action_event -> this.dispose());

			add(label_press_to_close);
			add(button_press_to_close);
		...
		@Override
		public void paint(Graphics g) {																// Отображение строки в диалоговом окне
			g.drawString("This is the dialog window", 20, 80);
		}

	}
	public static void main(String[] args) {
		new DialogWindowDemo("Menu Demo");
	}
}
